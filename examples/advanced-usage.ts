...
async function main() {
  ...
  const trainDataset = createTestDataset(30, 3, 32, 32);
  ...
  const { model: loadedModel } = loadModel(modelPath);
  
  // Create test dataset for evaluation
  const testDataset = createTestDataset(10, 3, 32, 32);
  
  const evaluator = new Trainer(loadedModel, {
    epochs: 1,
    batchSize: 4,
    learningRate: 0.001,
    optimizer: 'adam',
  ...
  const finetuneDataset = createTestDataset(15, 3, 32, 32);
  
  const finetuneTrainer = new Trainer(loadedModel, {
    epochs: 10,
    batchSize: 2,
  ...
  saveModel(loadedModel, finetunedPath, {
    learningRate: 0.0001,
    batchSize: 2,
    epochs: 10,
    optimizer: 'adam',
  ...
  const testImage = new Tensor(
    new Float32Array(3 * 32 * 32).map(() => Math.random()),
    [1, 3, 32, 32]
  );
